...
1. **Query Stream Generation:** A query stream generator script (`query_stream_generator.py`) was created to produce a stream of characters representing user queries. Different distributions of keywords were used to simulate various scenarios.
    
2. **Advertiser Data Generation:** An advertiser generator script (`advertiser_generator.py`) was implemented to create a set of advertisers with random budgets and keywords they bid on. This script also created a duplicate of the advertiser table (`advertisers_copy`) for the basic balance algorithm to operate on.
    
3. **Balance Algorithm Implementation:** Both the basic and modified balance algorithms were implemented in separate Python scripts (`balance_algorithm_basic.py` and `balance_algorithm.py`, respectively). These scripts read the query stream and advertiser data, performed the allocation logic, and stored the results in a `transactions` table.
